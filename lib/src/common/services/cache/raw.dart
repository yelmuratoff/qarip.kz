...
abstract class RawDynamicCachedFonts {
  ...
  static Future<FileInfo> cacheFont(
    ...
    final cacheKey = Utils.sanitizeUrl(url);
    ...
    final font = await DynamicCachedFontsCacheManager.getCacheManager(cacheKey)
        .downloadFile(
      url,
      key: cacheKey,
    );
  ...
  static Future<FileInfo> loadCachedFont(
    ...
    final fontBytes = await font.file.readAsBytes();

    final cachedFontBytes = ByteData.view(fontBytes.buffer);
  ...
  static Stream<FileInfo> loadCachedFamilyStream(
    ...
    for (final url in urls) {
      ...
      final fontBytes = await font.file.readAsBytes();
      final bytes = ByteData.view(fontBytes.buffer);

      fontLoader.addFont(Future.value(bytes));

      final totalItems = urls.length;
      final downloadedItems = urls.indexOf(url) + 1;

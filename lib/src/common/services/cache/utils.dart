// ignore_for_file: inference_failure_on_function_return_type, always_declare_return_types, type_annotate_public_apis, lines_longer_than_80_chars
...
void devLog(
  ...
  if (overrideLoggerConfig ?? Utils.shouldVerboseLog) {
    final message = messageList.join('\n');
    dev.log(
      message,
      name: kLoggerName,
    );
...
class _FontFileExtensionManager {
  ...
  bool matchesFileExtension(String path, Uint8List fileBytes) {
    ...
    final headerBytes = fileBytes.sublist(0, 5).toList();

    return _validExtensions.keys.contains(fontExtension) ||
        _validExtensions.values.any(
          (magicNumber) => listEquals(headerBytes, magicNumber),
        );
  }
}
...
class Utils {
  ...
  static String sanitizeUrl(String url) =>
      url.replaceAll(RegExp('[^A-Za-z0-9._-]'), '');
